...
# Re-exported so ``from lib.project_manager import ProjectOverview`` keeps working.
__all__ = [
    "GeneratedCharacterProfile",
    "GeneratedCharactersResult",
    "GeneratedPropProfile",
...
class ProjectManager:
    ...
    async def generate_episode_draft(
        ...
        *,
        ...
        """Delegate to :class:`ProjectAssetGenerator`."""
        from lib.project_asset_generator import ProjectAssetGenerator

        return await ProjectAssetGenerator(self).generate_episode_draft(project_name, episode, user_id=user_id)

    async def generate_characters(self, project_name: str, *, user_id: str = DEFAULT_USER_ID) -> dict:
        """Delegate to :class:`ProjectAssetGenerator`."""
        from lib.project_asset_generator import ProjectAssetGenerator

        return await ProjectAssetGenerator(self).generate_characters(project_name, user_id=user_id)

    async def generate_scenes(self, project_name: str, *, user_id: str = DEFAULT_USER_ID) -> dict:
        """Delegate to :class:`ProjectAssetGenerator`."""
        from lib.project_asset_generator import ProjectAssetGenerator

        return await ProjectAssetGenerator(self).generate_scenes(project_name, user_id=user_id)

    async def generate_props(self, project_name: str, *, user_id: str = DEFAULT_USER_ID) -> dict:
        """Delegate to :class:`ProjectAssetGenerator`."""
        from lib.project_asset_generator import ProjectAssetGenerator

        return await ProjectAssetGenerator(self).generate_props(project_name, user_id=user_id)

    async def generate_overview(self, project_name: str, *, user_id: str = DEFAULT_USER_ID) -> dict:
        """Delegate to :class:`ProjectAssetGenerator`."""
        from lib.project_asset_generator import ProjectAssetGenerator

        return await ProjectAssetGenerator(self).generate_overview(project_name, user_id=user_id)
