MESSAGES = {
    ...
    "connection_auth_failed": "连接已到达供应商，但 API Key 校验失败。请确认 Key 属于当前供应商、复制完整且未过期；如使用中转网关，请同时确认 Base URL 正确。",
    "connection_endpoint_not_found": "连接失败：接口地址不存在。请检查 Base URL 是否填写为该供应商的 OpenAI 兼容地址。",
    "connection_rate_limited": "连接失败：供应商限流或额度暂不可用，请稍后重试或检查账户额度。",
    "connection_provider_unavailable": "连接失败：供应商服务暂时不可用，请稍后重试。",
    "unsupported_test": "供应商 {provider_id} 暂不支持连接测试",
    "missing_credentials": "缺少凭证配置，请先添加密钥",
    "model_rule_github_url_invalid": "请填写有效的 GitHub Skill 文件、目录或仓库地址",
    "model_rule_github_not_found": "没有找到 SKILL.md，请确认 GitHub 地址指向 Skill 文件、Skill 目录或包含 SKILL.md 的仓库",
    "model_rule_github_fetch_failed": "GitHub Skill 导入失败: {message}",
    "model_rule_github_too_large": "Skill 文件超过 512KB，请拆分或精简后再导入",
    "model_rule_github_empty": "Skill 文件内容为空，请检查 GitHub 地址",
    # Assistant
    ...
    "unknown_content_type": "未知的内容类型: {content_type}",
    "unknown_content_mode": "未知的内容模式: {content_mode}",
    "unknown_generation_mode": "未知的生成模式: {generation_mode}",
