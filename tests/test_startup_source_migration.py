...
def test_startup_migration_creates_marker_after_run(tmp_path: Path):
    ...
    marker = project / ".scenelet" / "source_encoding_migrated"
...
def test_startup_migration_skips_legacy_arcreel_marker(tmp_path: Path):
    project = tmp_path / "p1"
    (project / "source").mkdir(parents=True)
    bad = project / "source" / "n.txt"
    bad.write_bytes(("第一章\n" * 30).encode("gbk"))
    ...
    assert bad.read_bytes().startswith("第一章".encode("gbk"))
    assert not (project / ".scenelet" / "source_encoding_migrated").exists()
...
def test_startup_migration_isolates_project_failures(tmp_path: Path, monkeypatch):
    ...
    summary = asyncio.run(_migrate_source_encoding_on_startup(tmp_path))
    assert (good / ".scenelet" / "source_encoding_migrated").exists()
    assert (bad / ".scenelet" / "source_encoding_migrated").exists()
    assert (bad / ".scenelet" / "migration_errors.log").exists()
