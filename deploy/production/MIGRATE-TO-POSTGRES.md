...
本文档适用于已使用默认 SQLite 部署 Scenelet、希望切换到 PostgreSQL 的场景。

## 前置条件

- 已安装 Docker 和 Docker Compose
- Scenelet 当前使用 SQLite 运行（数据库文件通常位于 `projects/.scenelet.db`；旧部署可能仍为 `projects/.arcreel.db`）
...
```bash
cp projects/.scenelet.db projects/.scenelet.db.bak
```
...
在 Scenelet 容器内使用 pgloader 将 SQLite 数据直接迁移到 PostgreSQL：
...
2. 恢复备份：`cp projects/.scenelet.db.bak projects/.scenelet.db`
